...
public class RepasoProgra2Application implements CommandLineRunner {
	...
	@Autowired
	private IEstudianteService personaService;
	...
	public void run(String... args) throws Exception {
		...
		for (int i = 1; i <= 10; i++) {
			...
			estu.setNombre("Persona_" + i);
			estu.setCedula("00" + i);
			estu.setFechaNacimiento(listaFechas.get(i-1));

			listaEstudiantes.add(estu);
			CompletableFuture<LocalDate> respuesta = this.personaService.agregar(estu);
